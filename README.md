...
NeuroForge is a local-first adaptive STEM reasoning trainer for iPhone, iPad, and Mac. This repository contains the native SwiftUI release-candidate implementation of the accompanying [Release 1.0 product specification](NeuroForge_Release_1.0_Product_Spec_System_Design_Development_Plan.md).

The product is designed around one safety boundary: optional model-assisted authoring can tailor personal practice, but deterministic code owns answer validation, partial credit, assessment evidence, scheduling, and progress. Core training remains available offline and when the external Question Writer Shortcut or its user-selected model is unavailable.
...
Topic-only sets can use the user-installed, user-configured Question Writer Shortcut. The learner chooses its **Use Model** provider; NeuroForge recommends ChatGPT but cannot attest the provider or model version. Imported sources default to **Offline only** and must be explicitly changed to **Question Writer + offline** before any excerpt is eligible. A source-backed run then identifies the selected sources and excerpt limits before asking for a second, per-run consent, and sends at most four excerpts, no more than 1,600 characters each or 4,800 characters total. Consent is bound internally to the one-run request ID and each excerpt's exact chunk identity, document version, and content hash; the expiring mailbox and source-link validator retain the exact bounded snapshots that were shown to the model. Original files, discarded excerpt tails, unselected text, answers, and progress history remain local. Returned sets pass local structure, topic-relevance, answer-leak, repetition, math-format, and source-link checks, then use recall → reveal → self-rate practice. Model answers are never treated as scoring authority or independently verified truth. A current-SDK adaptive-population gate refuses any future native provider until all seven 1,000-question floors and their signed-inventory coverage verify. The shipping target contains no native PCC provider or managed-PCC entitlement.

Offline authoring remains available across all eight question forms. It is a first-class question-bank route, not a model fallback disguised as generated content: every form becomes an immutable typed exercise with a machine-checkable response schema and deterministic scorer. Shortcut output and learner self-rating are never treated as scoring authority or proof of factual truth. For selected sources, the offline route supports complete-prose recall, while Question Writer can use bounded text extracted locally from every supported import format for open-response practice. Both routes fail closed instead of inventing filler.
...
Best-effort background preparation can create tomorrow’s canonical deterministic plan and maintain disposable local caches. It never invokes a native language model. The canonical plan, prompts, keys, rubrics, scorers, attempts, and progress remain deterministic.
...
Personal document practice is intentionally excluded from standardized estimates, baseline/holdout evidence, and progress claims.
...
This repository is a code release candidate, not a signed App Store binary. The following work requires developer-account, production-service, OS/SDK, or distribution access and remains release-blocking:
...
3. **Question Writer Shortcut verification:** inspect the published three-action `NeuroForge Private Authoring` Shortcut at its recorded public URL, then test it on an eligible physical device with ChatGPT as the recommended **Use Model** selection and with another available selection. Cover fresh setup, repeat topic-only and consented source-backed use, callback queue draining, cancellation, timeout, malformed output, unavailable provider, excerpt bounds and source-link tampering, and offline fallback. Confirm that the app never claims to attest the user-selected provider, original files remain local, and the archived app does not contain `com.apple.developer.private-cloud-compute`.
...
5. **Localization, catalog signing, and independent content review:** the code-level English/Japanese catalogs, placeholder checks, and literal inventory are complete, but a professional native-speaker review is still required. Before distribution, independently review the 7,000-question bank (including the 200 editorial retrieval concepts and all 800 computed retrieval instances across 80 calculation families), add its identities to the signed release inventory, bump the content version, and re-sign it in the approved offline environment—or obtain the signed release exception required by the product specification. Independently review every assessment answer, content-bank construct, evidence card, claim, and methodology reference before submission; the signed manifest proves integrity of the reviewed payload, not the correctness of unreviewed subject matter.
...
These gates are deliberately surfaced in the app’s capability status: saved preferences never masquerade as an available cloud or model service.
